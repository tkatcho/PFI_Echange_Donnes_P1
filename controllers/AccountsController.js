  ...
  login(loginInfo) {
    if (loginInfo) {
      ...
      this.HttpContext.response.badRequest(
        "Credential Email and password are missing."
      );
  }
  logout() {
  ...
  conflict() {
    if (this.repository != null) {
      ...
      if (id && email) {
        ...
        this.HttpContext.response.updated(
          this.repository.checkConflict(prototype)
        );
      } else this.HttpContext.response.updated(false);
    } else this.HttpContext.response.updated(false);
  }
  ...
  modify(user) {
    ...
    if (Authorizations.writeGranted(this.HttpContext, Authorizations.user())) {
      if (this.repository != null) {
        ...
        } else this.HttpContext.response.notFound();
      } else this.HttpContext.response.notImplemented();
    } else this.HttpContext.response.unAuthorized();
  }
  ...
  block(id) {
    if (Authorizations.writeGranted(this.HttpContext, Authorizations.admin())) {
      if (this.repository != null) {
        let user = this.repository.findByField("Id", id);
        if (user != null) {
          user.Authorizations = Authorizations.blocked();
